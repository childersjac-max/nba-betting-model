...
def build_team_logs(games):
    """Expand into one row per team per game."""
...
def compute_rolling_features(df, windows=[5, 10, 20]):
    """Rolling averages, win rates, and scoring variance per team."""
    frames = []
...
def build_game_level_features(df):
    """Join home and away team features into one row per game."""
...
def compute_elo(df, k=20, home_adv=100):
    """Margin-aware ELO ratings computed chronologically."""
...
def ev(p, o):
    ...
    return {
        ...
        "edge":         round(e, 3),
        "kelly_pct":    round(kelly(p, o)*100 if e > MIN_EDGE else 0, 1),
        "bet":          e > MIN_EDGE,
    }
...
if __name__ == "__main__":
    ...
    print(f"\nDone - predictions.json written ({len(out['games'])} games)")
